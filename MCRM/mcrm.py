...
class GRUCell(nn.Module):
    ...
    def __init__(self, input_size, hidden_dim, bias=True):
        ...
        self.bias = bias
        self.gru = nn.GRUCell(self.input_size, self.hidden_dim,self.bias)
    ...
    def forward(self, input_tensor, cur_state):
        ...
        #print(self.gru)
        hidden = self.gru(input_tensor,cur_state)
...
class MCRMCell(nn.Module):

    def __init__(self, input_size, hidden_dim,bias = True):
        super(MCRMCell,self).__init__()
        self.input_size = input_size
        self.hidden_dim = hidden_dim
        self.bias = bias

        self.Gates = nn.Linear(self.input_size + self.hidden_dim, 4 * self.hidden_dim,bias=self.bias)
        self.cell_core = GRUCell(2*self.hidden_dim,self.hidden_dim,self.bias) 
...
class MCRM(nn.Module):
    ...
    def __init__(self, input_size,hidden_dim, num_layers = 1,
        ...
        for i in range(0, self.num_layers):
            if i == 0:
                cell_list.append(MCRMCell(input_size=self.input_size,
                                          hidden_dim=self.hidden_dim[i],
                                          bias=self.bias))
            else:
                cell_list.append(MCRMCell(input_size=self.hidden_dim,
                                          hidden_dim=self.hidden_dim[i],
                                          bias=self.bias))
