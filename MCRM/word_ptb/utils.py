...
def data_generator(args):
    ...
    abs_path = abs_path.replace('/utils.py','')
    path = path.replace('.','')
    path = abs_path+path
    if os.path.exists(path+ "/corpus") and not args.corpus:
        corpus = pickle.load(open(path + '/corpus', 'rb'))
    else:
        corpus = Corpus(path)
        pickle.dump(corpus, open(path + '/corpus', 'wb'))
    return corpus
